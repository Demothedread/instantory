...
async def get_search_documents():
    ...
    try:
        ...
        async with vector_pool.acquire() as conn:
            ...
            if not document_ids:
                return (
                    jsonify({"results": [], "message": "No similar documents found"}),
                    200,
                )
...
async def search_documents():
    ...
    try:
        ...
        try:
            ...
            client = get_openai_client()
            if not client:
                logger.warning("OpenAI client unavailable, skipping vector search")
            else:
                response = await client.embeddings.create(
                    model="text-embedding-3-small", input=query
                )
            ...
            if not vector_pool:
                logger.warning(
                    "Vector database unavailable, falling back to text search"
                )
                    ...
                    async with metadata_pool.acquire() as metadata_conn:
                        ...
                        for doc in doc_rows:
                            ...
                            doc_dict["excerpt"] = extract_matching_excerpt(
                                doc_dict.get("extracted_text", ""), query
                            )
