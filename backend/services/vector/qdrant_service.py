...
class QdrantService:
    ...
    def __init__(self):
        ...
        # Import qdrant dependencies lazily to avoid httpx compatibility issues
        try:
            ...
            self.ResponseHandlingException = ResponseHandlingException
        ...
        self.url = os.getenv(
            "QDRANT_URL",
            "https://a155b5ab-3dca-44ae-a3b7-7c8e0c472bbd.europe-west3-0.gcp.cloud.qdrant.io:6333",
        )
        ...
        self.client = self.QdrantClient(url=self.url, api_key=self.api_key, timeout=30)
    ...
    async def initialize_collection(self) -> bool:
        ...
        try:
            ...
            if self.collection_name not in collection_names:
                ...
                await self._run_in_executor(
                    ...
                    self.VectorParams(
                        size=self.vector_size, distance=self.Distance.COSINE
                    ),
                )
        ...
        except (self.ResponseHandlingException, self.UnexpectedResponse) as e:
    ...
    async def store_document_vector(
        ...
        try:
            ...
            if result.status == self.UpdateStatus.COMPLETED:
                logger.debug("Successfully stored vector for document %s", document_id)
                return True
            else:
                logger.error(
                    "Failed to store vector for document %s: %s", document_id, result
                )
                return False
            ...
            logger.error(
                "Connection error storing document vector %s: %s", document_id, e
            )
            return False
        except (ValueError, TypeError) as e:
            logger.error(
                "Data validation error storing document vector %s: %s", document_id, e
            )
            return False
        ...
        try:
            ...
            if user_id is not None:
                query_filter = self.Filter(
                    must=[
                        self.FieldCondition(
                            key="user_id", match=self.MatchValue(value=user_id)
                        )
                    ]
                )
    ...
    async def delete_document_vector(self, document_id: int) -> bool:
        ...
        try:
            result = await self._run_in_executor(
                self.client.delete, self.collection_name, [document_id]
            )
            ...
            if result.status == self.UpdateStatus.COMPLETED:
                ...
                logger.error(
                    "Failed to delete vector for document %s: %s", document_id, result
                )
                return False
            ...
            logger.error(
                "Connection error deleting document vector %s: %s", document_id, e
            )
            return False
        except (ValueError, TypeError) as e:
            logger.error(
                "Data validation error deleting document vector %s: %s", document_id, e
            )
            return False
    ...
    async def delete_user_vectors(self, user_id: int) -> bool:
        ...
        try:
            ...
            if result.status == self.UpdateStatus.COMPLETED:
                ...
                logger.error(
                    "Failed to delete vectors for user %s: %s", user_id, result
                )
                return False
            ...
            logger.error(
                "Connection error deleting vectors for user %s: %s", user_id, e
            )
            return False
        except (ValueError, TypeError) as e:
            logger.error(
                "Data validation error deleting vectors for user %s: %s", user_id, e
            )
            return False
    ...
    async def get_document_vector(self, document_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                "Connection error retrieving document vector %s: %s", document_id, e
            )
            return None
        except (ValueError, TypeError, KeyError) as e:
    ...
    async def count_user_vectors(self, user_id: int) -> int:
        ...
        try:
            ...
            logger.error(
                "Connection error counting vectors for user %s: %s", user_id, e
            )
            return 0
        except (ValueError, TypeError) as e:
            logger.error(
                "Data validation error counting vectors for user %s: %s", user_id, e
            )
            return 0
...
# Global instance - created lazily to avoid import-time issues
_qdrant_service_instance = None


def get_qdrant_service():
    """Get or create the global QdrantService instance lazily."""
    global _qdrant_service_instance  # noqa: PLW0603
