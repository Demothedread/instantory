...
class DatabaseConfig:
    ...
    def __init__(self):
        ...
        self.vector_url = (
            self.database_urls["vector"]
            or self.database_urls["neon"]
            or self.database_urls["default"]
        )

        # Configure metadata database
        self.metadata_url = (
            self.database_urls["metadata"] or self.database_urls["default"]
        )

        logger.info(
            "Using vector database URL: %s", "configured" if self.vector_url else "none"
        )
        logger.info(
            "Using metadata database URL: %s",
            "configured" if self.metadata_url else "none",
        )
    ...
    def _parse_url(self, url: str) -> Dict[str, Any]:
        ...
        try:
            parsed = urlparse(url)
            logger.debug(
                "Parsed URL components - scheme: %s, username: %s, hostname: %s, port: %s, path: %s",
                parsed.scheme,
                parsed.username,
                parsed.hostname,
                parsed.port,
    ...
    async def get_pool(self, db_type: DatabaseType) -> Optional[asyncpg.Pool]:
        ...
        try:
            ...
            logger.info(
                "Attempting to create %s database pool with config: %s",
                db_type.value,
                safe_config,
            )
            ...
            self._pools[db_type] = await asyncpg.create_pool(
                ...
                max_size=10,
                command_timeout=60,
                server_settings={"application_name": f"bartleby_{db_type.value}"},
            )
            ...
            logger.error(
                "Connection error while creating %s database pool: %s", db_type.value, e
            )
            logger.error(
                "Config used (safe): %s",
                {k: v for k, v in config.items() if k != "password"},
            ...
            logger.error(
                "Postgres error while creating %s database pool: %s", db_type.value, e
            )
            logger.error(
                "Config used (safe): %s",
                {k: v for k, v in config.items() if k != "password"},
...
async def get_vector_pool() -> Optional[Union[asyncpg.Pool, "QdrantService"]]:
    ...
    try:
        ...
        if health.get("status") == "healthy":
            ...
            logger.warning(
                "Qdrant unhealthy, falling back to PostgreSQL: %s", health.get("error")
            )
