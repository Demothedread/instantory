...
class StorageConfig:
    ...
    def _validate_config(self) -> None:
        ...
        if self.storage_backend == 'vercel' and not self.vercel_blob_token:
            logger.warning("BLOB_READ_WRITE_TOKEN environment variable is missing but Vercel storage is selected")
            
        if self.storage_backend == 's3' and not all([self.s3_bucket, self.s3_access_key, self.s3_secret_key]):
            logger.warning("AWS S3 configuration is incomplete but S3 storage is selected")
        ...
        if self.storage_backend == 'vercel' and self.vercel_blob_token:
            if storage_type in [StorageType.IMAGE, StorageType.THUMBNAIL]:
                return 'vercel', {
                    ...
                    'content_type': file_type if storage_type == StorageType.IMAGE else 'image/jpeg'
                }
                
        if self.storage_backend == 's3' and all([self.s3_bucket, self.s3_access_key, self.s3_secret_key]):
